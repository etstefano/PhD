...
for yy in years_range:
    try:
        ...
        data_y[data_y.columns[-7:]].to_csv(myPath + "doc2vec2\\kpca\\" 
                                                        + str(yy) + "_kpca.csv")
        ...
        plt.show()
        plt.savefig(myPath + "doc2vec2\\kpca\\" + str(yy) + "_dendogram.png",
                                dpi = 300)
        plt.close()
...
import os
comms_files = list()
for file in os.listdir(myPath + "doc2vec2\\kpca\\"):
    if "csv" in file:
        comms_files.append(file)

for cf in comms_files:
    if '1962' in cf:
        df = pd.read_csv(myPath + "doc2vec2\\kpca\\" + cf, 
                         index_col = 0)
    else:
        df1 = pd.read_csv(myPath + "doc2vec2\\kpca\\" + cf, 
                         index_col = 0)
                         
        df = pd.concat([df, df1])

df.to_csv(myPath + "doc2vec2\\kpca\\" + "communities_kpca.csv")
