...
###############################################################################
myPath = "/home/uctqgur/Scratch/hansard/"
###############################################################################
...
def create_documents(h_temp, r):
    ...
    with open(myPath + 'all_speeches_' + str(ranges[r][0][0]) + '-' +
        str(ranges[r][1][0])+ '.txt', 'a') as f:
            ...
            for m in members:
                ...
                with open(myPath + 'sequence_' + str(ranges[r][0][0]) + "-" +
                    str(ranges[r][1][0])+ '.txt', 'a') as g: 
                        g.write(str(m) + '\n')

###############################################################################
##import entire dataset
hansard = pd.read_csv(myPath +
...
for r in range(0,len(ranges)):
    ...
    with open(myPath + "log.txt", "a") as log:
        log.write("working on years " + str(ranges[r][0][0]) + "-" +
                    str(ranges[r][1][0] + "\n")
    ...
    h_temp.to_csv(myPath + str(ranges[r][0][0]) + '-' +
